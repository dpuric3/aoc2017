const string = `una bokpr ftz ryw nau yknf fguaczl anu
...
function part2(string) {
    var passwords = string.split('\n');

    var sum = 0;
    debugger;

    var correctPasses = passwords.length;

    for (var k = 0; k < passwords.length; k++) {

        var passArr = passwords[k].split(/\s+/);
        var found = false;


        for (var i = 0; i < passArr.length; i++) {
            for (var j = 1; j < passArr.length; j++) {
                if (i !== j) {
                    var a = passArr[i];
                    var b = passArr[j];
                    if (regularize(a) == regularize(b)) {
                        found = true;
                        // console.log('-->' + passArr + '\n');
                        // console.log(a + " === " + b);
                        sum = sum + 1;
